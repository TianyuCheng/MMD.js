...
class this.MMD.PMDModel # export to top level
  ...
  getVertices: (buffer, view, offset) ->
    ...
    @vertices =
      for i in [0...length]
        new PMDVertex(buffer, view, offset + i * PMDVertex.size)
    offset += length * PMDVertex.size
  ...
  getMaterials: (buffer, view, offset) ->
    ...
    @materials =
      for i in [0...length]
        new PMDMaterial(buffer, view, offset + i * PMDMaterial.size)
    offset += length * PMDMaterial.size
  ...
  getBones: (buffer, view, offset) ->
    ...
    @bones =
      for i in [0...length]
        new PMDBone(buffer, view, offset + i * PMDBone.size)
    offset += length * PMDBone.size
  ...
  getRigidBodies: (buffer, view, offset) ->
    ...
    @rigid_bodies =
      for i in [0...length]
        new PMDRigidBody(buffer, view, offset + i * PMDRigidBody.size)
    offset += length * PMDRigidBody.size
  ...
  getJoints: (buffer, view, offset) ->
    ...
    @joints =
      for i in [0...length]
        new PMDJoint(buffer, view, offset + i * PMDJoint.size)
    offset += length * PMDJoint.size
...
PMDVertex.size = size_Float32 * 8 + size_Uint16 * 2 + size_Uint8 * 2 # 38
...
PMDMaterial.size = size_Float32 * 11 + size_Uint8 * 2 + size_Uint32 + size_Uint8 * 20 # 70
